...
public class GetMyPublishedLearningPathsQueryHandler : IRequestHandler<GetMyPublishedLearningPathsQuery, Result<PaginationDto<LearningPathListItemDto>>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public GetMyPublishedLearningPathsQueryHandler(IApplicationDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<Result<PaginationDto<LearningPathListItemDto>>> Handle(GetMyPublishedLearningPathsQuery request, CancellationToken cancellationToken)
    {
        Guid mentorId;
        try
        {
        ...
        var query = _context.LearningPaths
            ...
            .Include(lp => lp.User)
            .Where(lp => lp.UserId == mentorId && lp.Status == LearningPathStatus.Published.ToString())
            .AsQueryable();
